...
class Shopkeepers(Npc):
    ...
    def introduction(self):
        ...
        for i in self.items:
            returnValue.append(f"{self.items.index(i) + 1}. {(i["type"].upper() if i["name"].upper() == "POTION" else "") + " " + i["name"].upper()} for {i["cost"]} coins")
        return returnValue
    
    def sell(self, player, index):
        if player.coins >= self.items[index]["cost"]:
            player.givemymoney(-self.items[index]["cost"])
            returnValue = f"You have successfully purchased {(self.items[index]["type"].upper() if self.items[index]["name"].upper() == "POTION" else "") + " " + self.items[index]["name"].upper()}"
            if self.items[index] in item.weapon:
                player.inventory["weapon"].append(self.items[index])
                self.items.pop(index)
            else:
                player.inventory["potions"][self.items[index]["type"]] += 1
            return returnValue
        else:
            return f"You do not have enough coins to purchase {self.items[index]["name"].upper()}"

teacher1 = Teachers("images/teacher1.png", "Steve", item.weapon[3])
shop1 = Shopkeepers("images/dungeon3enemy3.png", "John", [item.weapon[3], item.potion[0], item.potion[1], item.potion[2]])

teacher2 = Teachers("images/teacher2.png", "Steve", item.weapon[2])
shop2 = Shopkeepers("images/dungeon3enemy3.png", "John", [item.weapon[2], item.potion[0], item.potion[1], item.potion[2]])

teacher3 = Teachers("images/teacher3.png", "Steve", item.weapon[1])
shop3 = Shopkeepers("images/dungeon3enemy3.png", "John", [item.weapon[1], item.potion[0], item.potion[1], item.potion[2]])
